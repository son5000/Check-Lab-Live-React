...
function transformTypescript(content){
  content = removeTypeDeclarations(content);

  content = content.replace(/import\s*\{([^}]*)\}/g, (match, inner) => {
    const cleaned = inner
      .replace(/\btype\s+([A-Za-z0-9_$]+)/g, '$1')
      .replace(/,\s*}/, '}')
      .replace(/\{\s*,/, '{');
  ...
  content = content.replace(/\bdeclare\b\s*/g, '');
  content = content.replace(/(const|let|var)\s+([A-Za-z0-9_$]+)\s*:\s*[A-Za-z0-9_$.<>\[\]| ,]+(?=\s*=)/g, '$1 $2');
...
async function copyAndTransform(srcRoot, destRoot){
  async function walk(dir){
    ...
    for(const e of entries){
      ...
      if(lower === 'tsconfig.json' || lower === 'next-env.d.ts') continue;
...
async function main(){
  ...
  if(!src || !dest){
    console.error('Usage: node convert-ts-to-js.js <source-root> <dest-root>');
    process.exit(1);
  }
  ...
  await copyAndTransform(absSrc, destRoot);
  console.log('Done.');
}
