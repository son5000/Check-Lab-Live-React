...
import { TemperatureMetricCard, UltrasoundMetricCard, } from "./summary/value-metrics";
import { ThresholdEditor } from "./summary/threshold-editor";
const DETECTION_AREA_SLIDE_INTERVAL_MS = 5000;
const DEFAULT_SUMMARY_METRIC_ORDER = [
    "ultrasound",
    "temperature",
    "assetPart",
];
...
export function AssetSummaryPanel({ averageTemperature, assetParts, assetPartStates, assetThresholds, assetJudgement, asset, isSimplified = false, isThresholdSaving = false, temperatureMax, temperatureMin, thresholdSaveError, ultrasoundAverageDb, ultrasoundDetectionCount, ultrasoundMax, selectedAssetPartId, onAssetPartSelect, onAssetThresholdSave, onThresholdEditorDirtyChange, onStartAssetPart, variant = "full", }) {
    ...
    if (variant === "metrics") {
        ...
        <div className="AssetDetailSummaryPanel AssetDetailSummaryPanel__container-1 mb-2 flex min-w-0 items-center justify-between gap-2">
          <div className="AssetDetailSummaryPanel AssetDetailSummaryPanel__container-2 min-w-0">
            <Gauge aria-hidden="true" className="AssetDetailSummaryPanel AssetDetailSummaryPanel__title-1 h-4 w-4 text-muted-foreground"/>
          </div>
          <div className="AssetDetailSummaryPanel AssetDetailSummaryPanel__container-6 flex shrink-0 items-center gap-1.5">
            <button type="button" className={cn("AssetDetailSummaryPanel AssetDetailSummaryPanel__button-1 grid h-7 w-7 shrink-0 place-items-center rounded-md border text-muted-foreground transition hover:bg-accent hover:text-foreground", isThresholdEditorOpen
                ? "border-primary bg-primary/10 text-primary"
                : "border-border bg-background")} aria-label="설비 요약 설정" aria-expanded={isThresholdEditorOpen} aria-controls="asset-threshold-popover" title="설비 요약 설정" onClick={() => setIsThresholdEditorOpen((isOpen) => !isOpen)}>
              <SlidersHorizontal className="AssetDetailSummaryPanel AssetDetailSummaryPanel__icon-1 h-3.5 w-3.5" aria-hidden="true"/>
            </button>
            <span className={cn("AssetDetailSummaryPanel AssetDetailSummaryPanel__label-1 shrink-0 rounded-md border px-2 py-1 text-[11px] font-semibold", judgementClassName[assetJudgement])}>
    ...
    return (<section className={cn("AssetDetailSummaryPanel AssetDetailSummaryPanel__section-1 min-h-0 min-w-0 overflow-hidden rounded-md border border-border bg-card text-card-foreground", !isSimplified ? "grid h-full grid-rows-3" : "flex flex-col")}>
      {/* ── 1/3: 초음파 카드 ── */}
      <div className="AssetDetailSummaryPanel AssetDetailSummaryPanel__row-1 min-h-0 overflow-hidden border-b border-border p-2">
        <UltrasoundMetricCard averageDb={ultrasoundAverageDb} peakDb={ultrasoundMax.peakDb} dominantFrequencyKHz={ultrasoundMax.dominantFrequencyKHz} frequencyBandKHz={ultrasoundMax.frequencyBandKHz} detectionCount={ultrasoundDetectionCount} threshold={assetThresholds?.ultrasoundDb} isExceeded={isUltrasoundExceeded}/>
      </div>

      {/* ── 2/3: 온도 카드 ── */}
      <div className="AssetDetailSummaryPanel AssetDetailSummaryPanel__row-2 min-h-0 overflow-hidden border-b border-border p-2">
        <TemperatureMetricCard averageTemperature={averageTemperature} temperatureMax={temperatureMax} temperatureMin={temperatureMin} threshold={assetThresholds?.temperature} isExceeded={isTemperatureExceeded}/>
      </div>
...
function AssetAssetInfoSection({ asset, }) {
    ...
    return (<section className="AssetAssetInfoSection AssetAssetInfoSection__section-1 flex min-h-0 min-w-0 flex-col overflow-hidden rounded-md border border-border bg-background p-1.5" aria-label="설비 기본 정보">
      <div className="AssetAssetInfoSection AssetAssetInfoSection__header-1 mb-1.5 flex min-w-0 items-center justify-between gap-2">
        ...
        <span className={cn("AssetAssetInfoSection AssetAssetInfoSection__status-1 shrink-0 rounded-sm border px-1.5 py-0.5 text-[10px] font-semibold", operationStateClassName[operationState])}>
      ...
      <dl className="AssetAssetInfoSection AssetAssetInfoSection__list-1 grid min-h-0 flex-1 auto-rows-min gap-1 overflow-y-auto overscroll-contain pr-1 [scrollbar-gutter:stable] [scrollbar-width:thin]">
        {assetInfoItems.map((item) => (<div key={item.label} className="AssetAssetInfoSection AssetAssetInfoSection__row-1 grid min-w-0 grid-cols-[minmax(6.75rem,7.75rem)_minmax(0,1fr)] items-center gap-2 rounded-sm border border-border/70 bg-card px-2 py-1.5">
            ...
            <dd className="AssetAssetInfoSection AssetAssetInfoSection__value-1 min-w-0 truncate text-right font-mono text-[11px] font-bold text-foreground" title={item.value}>
              {item.value}
            </dd>
          </div>))}
      </dl>
...
function AssetPartMetricCarousel({ activeIndex, parts, partStates, slideOptions, }) {
    ...
    const isTemperatureExceeded = isCriticalThresholdExceeded(temperatureValue, activePart.thresholds.temperature, activePart.thresholds.temperatureCritical);
    const isUltrasoundExceeded = isCriticalThresholdExceeded(ultrasoundValue, activePart.thresholds.ultrasoundDb, activePart.thresholds.ultrasoundCriticalDb);
    const accentClassName = judgement === "abnormal"
        ? "bg-red-500"
        : judgement === "caution"
            ? "bg-amber-500"
            : "bg-emerald-500";
      ...
      <div className="grid min-h-0 flex-1 grid-cols-2 overflow-hidden">
        <div className="relative isolate flex min-h-0 min-w-0 flex-col justify-between overflow-hidden border-r border-border/40 p-2">
          ...
          <div className="mt-1 flex min-w-0 items-center justify-between gap-1">
            ...
            <span className={cn("shrink-0 rounded-sm border px-1.5 py-0.5 text-[10px] font-semibold", judgementClassName[judgement])}>
        ...
        <dl className="grid h-full min-h-0 min-w-0 grid-rows-4 gap-1 px-2 py-2">
          <DetectionMetricInfoRow highlighted={isTemperatureExceeded} label="온도" value={`${roundOne(temperatureValue)} / ${roundOne(activePart.thresholds.temperature)} ℃`}/>
          <DetectionMetricInfoRow highlighted={isUltrasoundExceeded} label="초음파" value={`${roundOne(ultrasoundValue)} / ${roundOne(activePart.thresholds.ultrasoundDb)} dB`}/>
          <DetectionMetricInfoRow label="주파수" value={`${activePartState?.dominantFrequencyKHz ?? 0} kHz`}/>
          <DetectionMetricInfoRow label="지점" value={formatAssetPartScope(activePart)}/>
        </dl>
      </div>
    </article>);
}
function AssetPartMiniMap({ part }) {
    ...
    return (<div className="AssetPartMiniMap AssetPartMiniMap__container-1 relative min-h-0 flex-1 overflow-hidden rounded-md border border-border/60 bg-card [background-image:linear-gradient(90deg,color-mix(in_oklch,var(--muted-foreground)_18%,transparent)_1px,transparent_1px),linear-gradient(color-mix(in_oklch,var(--muted-foreground)_18%,transparent)_1px,transparent_1px)] [background-size:18px_18px]">
      {part.roi ? (<span className="AssetPartMiniMap AssetPartMiniMap__roi-1 absolute rounded-sm border-2 border-cyan-500 bg-cyan-500/15" style={{
                height: `${part.roi.height}%`,
                left: `${part.roi.x}%`,
                top: `${part.roi.y}%`,
                width: `${part.roi.width}%`,
            }}/>) : null}
      {part.points.map((point, index) => (<span key={point.id} className="AssetPartMiniMap AssetPartMiniMap__point-1 absolute grid h-4 w-4 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-cyan-200 bg-cyan-500 text-[9px] font-bold text-white shadow-sm" style={{
                left: `${point.x}%`,
                top: `${point.y}%`,
            }}>
          {index + 1}
...
function AssetPart3DMiniMap({ part }) {
    ...
    if (target?.previewImageDataUrl) {
        return (<div className="AssetPart3DMiniMap AssetPart3DMiniMap__container-1 relative min-h-0 flex-1 overflow-hidden rounded-md border border-cyan-500/30 bg-slate-950">
        <img alt={`${part.name} 3D 캡쳐`} className="AssetPart3DMiniMap AssetPart3DMiniMap__image-1 absolute inset-0 h-full w-full object-contain" src={target.previewImageDataUrl}/>
    ...
    return (<div className="AssetPart3DMiniMap AssetPart3DMiniMap__container-1 relative min-h-0 flex-1 overflow-hidden rounded-md border border-cyan-500/30 bg-slate-950">
      ...
      {part.roi ? (<span className="AssetPart3DMiniMap AssetPart3DMiniMap__roi-1 absolute rounded-sm border-2 bg-cyan-300/15 shadow-[0_0_14px_rgba(34,211,238,0.32)]" style={{
      ...
      {point ? (<span className="AssetPart3DMiniMap AssetPart3DMiniMap__point-1 absolute grid h-4 w-4 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-white/70 text-[9px] font-bold text-white shadow-[0_0_16px_rgba(34,211,238,0.42)]" style={{
...
function DetectionMetricInfoRow({ highlighted, label, value, }) {
    return (<div className={cn("DetectionMetricInfoRow DetectionMetricInfoRow__row-1 flex min-h-0 min-w-0 items-center justify-between gap-1 overflow-hidden rounded-[4px] border border-border/45 border-l-2 bg-background/55 px-1.5 py-1.5", highlighted
      ...
      <dd className={cn("DetectionMetricInfoRow DetectionMetricInfoRow__value-1 min-w-0 truncate text-right font-mono text-[12px] font-black leading-none text-foreground", highlighted && "text-red-500 dark:text-red-400")}>
        {value}
      </dd>
    </div>);
}
...
const operationStateClassName = {
    ...
    비가동: "border-slate-500/30 bg-slate-500/10 text-slate-700 dark:text-slate-300",
};
