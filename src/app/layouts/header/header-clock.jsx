...
export function HeaderClock({ currentDate, currentTime }) {
    return (<div className="HeaderClock HeaderClock__container-1 hidden h-10 min-w-[21rem] shrink-0 items-center justify-center gap-4 px-2 text-foreground md:flex" aria-label={`현재 날짜 ${currentDate}, 현재 시간 ${currentTime}`}>
      <div className="HeaderClock HeaderClock__container-2 flex min-w-0 items-baseline gap-2">
        ...
        <span className="HeaderClock HeaderClock__label-1 max-w-48 truncate text-sm font-bold leading-none text-foreground" title={currentDate}>
          {currentDate}
        </span>
      </div>
      ...
      <div className="HeaderClock HeaderClock__container-3 flex items-baseline gap-2 text-right">
        ...
        <span className="HeaderClock HeaderClock__label-2 w-[6.25rem] font-mono text-lg font-bold leading-none text-foreground tabular-nums" title={currentTime}>
          {currentTime}
        </span>
      </div>
    </div>);
