...
export function MainHeader({
  ...
  return (
    <header className="MainHeader MainHeader__header-1 grid h-14 shrink-0 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 overflow-hidden border-b border-border bg-background px-2.5 md:px-3 xl:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
      <div className="MainHeader MainHeader__container-1 flex min-w-0 items-center gap-2">
        <HeaderIconButton label={menuLabel} onClick={onMenuToggle}>
          <Menu
            className="MainHeader MainHeader__icon-1 h-4 w-4 md:hidden"
            aria-hidden="true"
          />
          <PanelLeftClose
            className={cn(
              "MainHeader MainHeader__icon-2 hidden h-4 w-4 md:block",
